...
void BreathAnimator::initialize(std::shared_ptr<ChanMap> p_chan_map)
{
  ...
  fpath package_path = ament_index_cpp::get_package_share_directory("simple_breath_animator");
  fpath tree_path = package_path / "behavior_trees" / "breath_anim.xml";
  ...
  factory.registerBuilder<HasSpeakTask>(
    ...
    [this](
      ...
      const BT::NodeConfiguration & config)
    {
      return std::make_unique<HasSpeakTask>(
        name, config,
        std::bind(&BreathAnimator::has_speak_task_, this));
    });

  ChannelCore1D::SharedPtr p_diaphragm_chan = get_1d_chan_(*p_chan_map, "diaphragm");
...
PLUGINLIB_EXPORT_CLASS(expression::BreathAnimator, expression::Animator)
